import { UsuariosRolesQueries } from "../../queries/gestionUsuarios/UsuariosRolesQueries";
...
export const UsuariosRolesAJAXRequest = {
  obtenerUsuariosRoles: async () => {
    const { data } = await AJAXRequest(URI, {
      query: UsuariosRolesQueries.UsuariosRolesObtencion
    });
    return data.data.leerUsuariosRoles;
  },
  obtenerRoles: async () => {
    const { data } = await AJAXRequest(URI, {
      query: UsuariosRolesQueries.RolesObtencion
    });
    return data.data.leerRoles;
  },
  crearUsuarioRol: async (item) => {
    const { data } = await AJAXRequest(URI, {
      query: UsuariosRolesQueries.UsuariosRolesCreacion,
      variables: {
  ...
  eliminarUsuarioRol: async (item) => {
    const { data } = await AJAXRequest(URI, {
      query: UsuariosRolesQueries.UsuariosRolesEliminacion,
      variables: {
  ...
  modificarUsuarioRol: async (item1, item2) => {
    const { data } = await AJAXRequest(URI, {
      query: UsuariosRolesQueries.UsuariosRolesActualizacion,
      variables: {
        rolNuevoId: item1,
